...
def get_data(num_classes=250, res=128, flip=True, color_invert=True, center=False,rotate_3=True):
    """
    ...
    #root_dir = "data/png{}/".format("" )

    num_train = 96 if flip else 48
    num_train =3*(num_train) if rotate_3 else num_train
    num_val = 16
    num_test = 16
    ...
    for node in sorted(os.listdir(root_dir)):
        ...
        for im_file in sorted(os.listdir(label_path)):
            ...
            if num_images < num_train:
                ...
                train_index += 1
                if flip:
                    flipped=np.flip(im_data,axis=1)
                    X_train[train_index] = flipped
                    train_index += 1
                    num_images += 1
                if rotate_3:
